...
public class ServerV2 {
    ...
    //this is the second version of the algorithm. It is designed to input ones into the first couple of threads
    //and the remaining tasks are allocated to the last thread 
    public static int[] tasksPerThread(int numberOfTasks, int numberOfThreads) {

        int[] valuesPerThread = new int[1000];
        int remainder = numberOfTasks - numberOfThreads + 1;
        for(int i = 0; i <= numberOfThreads - 2 && i <= numberOfTasks - 1; i++){
            valuesPerThread[i]++;
        }
        if(remainder > 0) {
            valuesPerThread[numberOfThreads - 1] += remainder;
        }
    ...
    static void computeThreads(String function, int numberOfThreads, int[][] threadsTasks, int[] nrThreadsTasks) {
        ...
        for(int k = 0; k <= numberOfThreads - 1; k++) { // create the kth thread with the following set of parameters
        ...
        for(int k = 0; k <= numberOfThreads - 1; k++) {
            for(int t = 0; t <= nrThreadsTasks[k]; t++) { //this for loop might seem redundat, but without it we might never get the output we want in time. This way we assure
                result = threadComputation[k].getResult(); //that we get the desired result eventually in this aproximated nrThreadsTasks attempts
                resultSize = threadComputation[k].getResultSize(); 
            }
            if(threadComputation[k].getStatus()) { //we check if the calculation has completed in that thread
                listUnion(result, resultSize); //if so unite this present result to the final result
                threadComputation[k].joinThread(); //and close the thread
                //If this were C++ I would have a deconsturcur here. But Java is a garbage collector. Figuratively and literally
            }
            else
